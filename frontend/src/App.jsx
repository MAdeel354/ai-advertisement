...
function App() {
  ...
  const handleGenerate = async (e) => {
    ...
    try {
      ...
      const requestData = {
        prompt: prompt.trim(),
        generate_video: true // Generate both logo and video
      }

      const result = await callBackendAPI(requestData)
      ...
      if (result.success && result.data.success) {
        ...
        if (result.data.logo_url) {
          setGeneratedLogo(`${baseUrl}${result.data.logo_url}?t=${timestamp}`)
        }
        if (result.data.video_url) {
          setGeneratedVideo(`${baseUrl}${result.data.video_url}?t=${timestamp}`)
        }

        console.log('Successfully generated content via backend API')
      } else {
            ...
            <form onSubmit={handleGenerate} className="prompt-form">
              ...
              {error && (
                <div className="error-message">
                  <p>⚠️ {error}</p>
                  <small>Make sure the backend server is running on http://localhost:8000 for AI generation.</small>
                </div>
              )}
            </form>
